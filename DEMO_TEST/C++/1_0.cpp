...
//图1.1显示了一个计算机处理敲好两个任务时的理想情景，每个任务被分为10个相等大小的块，在一个双核机器(具有两个处理核心)上，每个任务可以在各自的处理器核心上执行，在单核机器上做任务切换时，每个任务的块被分为10个相等的大小的块，在一个双核机器(具有两个处理器核心)上，每个任务可以在各自的处理核心上执行，在单核机器上做任务切换时，每个任务的块交织进行，但它们中间有一小段分割(图中所示灰色分割条的厚度大于双核机器的分隔条的厚度大于双核机器的分隔条)，为了实现交织进行，系统每次从一个任务切换到另一个时都需要切换一次上下文，任务切换也有时间开销，进行上下文的切换时，操作系统必须为当前运行的任务保存CPU的状态和指令指针，并计算出要切换到哪个任务，并为即将切换到的任务重新加载处理器状态，然后，CPU可能要讲新任务的指令和数据内存载入到缓存中，这会阻止CPU执行任何指令，从而造成的更多的延迟

//有些处理器可以在一个核心上执行多个线程，但硬件并发在多处理器或多核系统上效果更加显著，硬件线程最重要的因素是数量，也就是硬件上可以并发运行多少独立的任务，即便是具有真正硬件并发的系统，也很容易拥有比硬件"可并行最大任务数"还要多的任务需要执行，所以任务切换在这些情况加仍然使用，例如，在一个典型的台式计算机上可能会有成百上千的任务在运行，即便是在计算机处于空闲时，还是会有后台任务在运行，正式任务切换使得这些后台任务可以运行，并使得你可以同时运行文字处理器，编译器，编辑器和web浏览器(或其他应用的组合)，图1.2显示了四个任务在双核处理器上的任务切换，仍然是将任务整齐的划分为同等大小块的理想情况，实际上，许多阴虚会使得分割不均和调度不规则，部分因素将在第8章中讨论，那时我们再来看一看影响并行代码性能的阴虚
...
//当然，以上的机制也不是一无是处，操作系统在进程间提供的附加保护操作和更高级别的通信机制，意味着可以更容易编写安全的并发代码，实际上，在类似于Erlang编程环境中，讲进程作为并发的基本构造块
...
//主要原因有两个:关注点分离(SOC)和性能，事实上，他们应该是使用并阿福的唯一原因，如果你观察的足够仔细，所有的因素都可以归结到其中的一个原因(或者可能是两个都有，当然，除了像"就因为我愿意"这样的原因之外)
...
//在这种情况下，线程的熟练不再依赖CPU中的可用内核的数量，因为对线程的划分是基于概念上的设计，而不是一种增加吞吐量的尝试
...
//当客户端/服务器(C/S)应用在服务器端为每一个链接启动一个独立的线程，对于少量的链接是可以正常公祖ode，但当同样的技术用于需要处理大量链接的高需求服务器时，也会因为线程太多而耗尽系统资源，在这种情景下，谨慎地使用线程池可以对性能产生优化(参见第9章)
